...
function OrderDetailModal({ orderId, onClose }: DetailModalProps) {
  ...
  const sc = statusConfig[localStatus] ?? statusConfig['جديد'];
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-start gap-4 bg-gradient-to-l from-blue-50 to-indigo-50 rounded-2xl p-4">
              {order.profiles?.avatar_url ? (
                <img src={order.profiles.avatar_url} alt={customerName}
              ...
              <div className="flex-1 min-w-0">
                ...
                {addr?.street && (
                  <p className="text-xs text-gray-400 mt-0.5">
                    {[addr.street, addr.building, addr.floor ? `طابق ${addr.floor}` : '', addr.apartment ? `شقة ${addr.apartment}` : ''].filter(Boolean).join('، ')}
                  </p>
                )}
              </div>
              <button
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 flex-wrap mt-0.5">
                          {item.product_variants?.variant_name && (
                            <span className="text-xs text-blue-600 font-semibold">{item.product_variants.variant_name}</span>
                          )}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><DollarSign size={11} />تكلفة الشحن</p>
                <p className="text-sm font-bold text-gray-800">{order.shipping_cost.toLocaleString()} {currency}</p>
              </div>
              <div className="bg-gray-50 rounded-xl p-3">
                ...
                <p className="text-sm font-bold text-red-500">
                  ...
                  {order.coupons?.code && <span className="text-xs font-mono text-amber-600 mr-1">({order.coupons.code})</span>}
                </p>
              </div>
              <div className="bg-blue-50 rounded-xl p-3">
                <p className="text-xs text-gray-500 mb-1">الإجمالي</p>
                <p className="text-lg font-black text-blue-700">{order.total_price.toLocaleString()} {currency}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><CreditCard size={11} />طريقة الدفع</p>
                <p className="text-sm font-semibold text-gray-800">
                  {order.payment_methods?.method_name ?? '—'}
                  {order.payment_methods?.provider && <span className="text-xs text-gray-400 mr-1">({order.payment_methods.provider})</span>}
                </p>
              </div>
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><Calendar size={11} />تاريخ الطلب</p>
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 py-2 rounded-xl text-xs font-semibold border-2 transition-all flex items-center justify-center gap-1 ${
                        active ? `${cfg.cls} border-current` : 'border-gray-100 text-gray-500 hover:border-gray-300 bg-white'
                      }`}
                    >
...
export default function Orders() {
  ...
  /* ── Client-side search (name / phone / id / city) ── */
  const filtered = orders.filter(o => {
    ...
    return (
      ...
      (o.shipping_address?.city ?? '').includes(q) ||
      (o.shipping_address?.governorate ?? '').includes(q)
    );
  });
  ...
  /* ── Status counts for pill bar ── */
  const statusCounts = ALL_STATUSES.reduce((acc, s) => {
    acc[s] = orders.filter(o => o.status === s).length;
    return acc;
  }, {} as Record<string, number>);

  const currency = orders[0]?.countries?.currency_symbol ?? 'ج.م';
      ...
      <div className="fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="section-subtitle">متابعة الطلبات وحالاتها وتفاصيلها — بيانات حقيقية من Supabase</p>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 mb-5 flex-nowrap">
          <button
            ...
            className={`flex-shrink-0 px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${
              filterStatus === 'الكل' ? 'bg-blue-700 text-white' : 'bg-white text-gray-600 hover:bg-gray-100 shadow-sm'
            }`}
          >
            الكل ({orders.length})
          </button>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
              ...
              {inTransit > 0 && (
                <span className="flex items-center gap-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-xl">
                  <Truck size={12} /> {inTransit} طلب جاري الشحن
                </span>
              )}
            </div>
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm mt-1">لم يسجل أي طلب بعد</p>
              </div>
            ) : (
                    ...
                    {filtered.map(order => {
                      ...
                      return (
                        <tr key={order.id} className="table-row">
                          ...
                          {/* Products (summary) - no items join on list */}
                          <td className="px-4 py-3 text-sm text-gray-500">
                            <button
                              onClick={() => setSelectedOrderId(order.id)}
                              className="flex items-center gap-1 text-xs text-blue-600 hover:underline font-semibold"
                            >
                              <Eye size={11} /> عرض العناصر
                            </button>
                          </td>

                          {/* Total */}
                          <td className="px-4 py-3">
                            <p className="text-sm font-bold text-gray-800">{order.total_price.toLocaleString()} {order.countries?.currency_symbol ?? 'ج.م'}</p>
