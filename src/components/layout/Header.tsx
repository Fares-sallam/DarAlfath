...
import { Bell, Search, Menu, ChevronDown, User, Settings, LogOut, ShoppingCart, Package, AlertTriangle, UserPlus } from 'lucide-react';
...
function NotifIcon({ type }: { type: string }) {
  if (type === 'order' || type === 'new_order') return <ShoppingCart size={14} className="text-blue-600" />;
  if (type === 'inventory' || type === 'low_stock') return <AlertTriangle size={14} className="text-amber-500" />;
  if (type === 'customer' || type === 'new_user') return <UserPlus size={14} className="text-green-600" />;
...
export default function Header({ onMenuToggle, title, subtitle }: HeaderProps) {
  ...
  return (
    <header className="bg-white border-b border-gray-100 h-20 flex items-center px-6 sticky top-0 z-30 shadow-sm">
      <div className="flex items-center gap-4 flex-1">
        {/* Mobile Menu */}
        <button onClick={onMenuToggle} className="lg:hidden p-2 rounded-xl hover:bg-gray-100 transition-colors">
            ...
            <div className="absolute left-0 top-14 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {unreadCount > 0 && (
                    <span className="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded-full">{unreadCount} جديد</span>
                  )}
                  {unreadCount > 0 && (
                ...
                <ul className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                  {notifications.map(n => (
                    <li
                      ...
                      className={`px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer ${!n.is_read ? 'bg-blue-50/40' : ''}`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm ${!n.is_read ? 'font-semibold text-gray-800' : 'text-gray-600'} truncate`}>
                            {n.title}
                          </p>
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-gray-800 leading-tight">{user?.fullName || 'مدير النظام'}</p>
              <p className="text-xs text-gray-500">مدير دار الفتح</p>
            </div>
            {user?.avatarUrl ? (
              ...
              <div className="w-9 h-9 rounded-xl bg-[#0B1F4D] flex items-center justify-center border-2 border-blue-100 overflow-hidden">
                {logoUrl
                  ...
                  : <span className="text-[#D4AF37] font-bold text-sm">{(user?.fullName || 'م')[0]}</span>
                }
              </div>
            )}
          ...
          {showProfile && (
            <div className="absolute left-0 top-14 w-52 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-gray-100">
                <p className="font-bold text-sm text-gray-800">{user?.fullName || 'مدير النظام'}</p>
                <p className="text-xs text-gray-400 truncate">{user?.email}</p>
              </div>
              <ul className="py-1">
                <li>
                  <button
                    onClick={() => { setShowProfile(false); navigate('/settings', { state: { section: 'profile' } }); }}
